/** Shared surface-tint defaults — keep in sync with scripts/color-utils.js */
export const SURFACE_TINT_DEFAULTS = {
  targetLightness: 0.07,
  maxSaturation: 0.8,
  tintWeight: 1,
};
/** Matches DetailOverlay header text scrim base */
...
export function compositeScrim(src: Rgb, alpha = 0.95): Rgb {
  const inv = 1 - alpha;
  return {
...
/** Runtime fallback when cached bottom colors used the old bright averaging algorithm. */
export function toDarkSurfaceTint(
  sourceRgb: Rgb,
  opts: { targetLightness?: number; maxSaturation?: number; tintWeight?: number } = {},
): Rgb {
  const {
...
export function normalizeBottomColor(entry: Rgb & { bottom?: Rgb; bottomVersion?: number }): Rgb {
  const raw = entry.bottom ?? entry;
  if (entry.bottomVersion === BOTTOM_COLOR_VERSION) {
    return raw;
  }
  return toDarkSurfaceTint(raw);
}
